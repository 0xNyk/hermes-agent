  ...
  flake.nixosModules.default = { config, lib, pkgs, ... }:
    ...
    in {
      options.services.hermes-agent = {
        ...
        gateway = {
          enable = lib.mkEnableOption "Hermes Agent messaging gateway (includes cron scheduler)";
        };
      };
...
YAML
        ...
        # Gateway service (includes cron scheduler as a background thread)
        systemd.services.hermes-agent-gateway = lib.mkIf cfg.gateway.enable {
          description = "Hermes Agent messaging gateway (includes cron scheduler)";
          wantedBy = [ "multi-user.target" ];
